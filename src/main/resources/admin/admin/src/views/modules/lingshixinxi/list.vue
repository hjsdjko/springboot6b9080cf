		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">零食名称</label>
						<el-input v-model="searchForm.lingshimingcheng" placeholder="零食名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">零食品牌</label>
						<el-input v-model="searchForm.lingshipinpai" placeholder="零食品牌" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
			...
			<div :style='{"width":"100%","padding":"0","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="lingshitupian" width="200" label="零食图片">
						<template slot-scope="scope">
							<div v-if="scope.row.lingshitupian">
								<img v-if="scope.row.lingshitupian.substring(0,4)=='http'" :src="scope.row.lingshitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.lingshitupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.lingshitupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.lingshitupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.lingshipinpai!='' && this.searchForm.lingshipinpai!=undefined){
            params['lingshipinpai'] = '%' + this.searchForm.lingshipinpai + '%'
          }
